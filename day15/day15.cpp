...
struct  F
{
    ...
    {
        return (!(a->cost + (v.size() - a->x) + (v.size() - a->y) < b->cost + (v.size() - b->x) + (v.size() - b->y)));
    }
};
...
void movements(const node & n, std::string m)
{
    ...
    s = (100*tmp->y) + tmp->x;
    tmp->cost += (v[tmp->y % v.size()][tmp->x %v.size()] -48); 
...
void    A_star(){
    ...
    for (;n->x != v.size() -1 || n->y != v.size()-1; n = &(*open.top()))
    {
        open.pop();
        if (((!n->p) || n->p->x != n->x +1) && n->x < v.size() - 1)
            movements(*n, "R");
        if (((!n->p) || n->p->y != n->y -1) && n->y > 0)
            movements(*n, "U");
        if (((!n->p) ||n->p->y != n->y +1) && n->y < v.size() -1)
            movements(*n, "D");
